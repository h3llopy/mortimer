...
class mrp_bom(models.Model):
    ...
    def _add_product(self, barcode):
        ...
        if self:
            ...
            elif self.env.user.company_id.sudo().sh_bom_barcode_scanner_type == "both":            
                if is_multi_barcode:        
                    search_lines = self.bom_line_ids.filtered(lambda ol: ol.product_id.barcode == barcode or ol.product_id.default_code == barcode)   
                    if not search_lines:
                        for line in self.bom_line_ids:
                            if line.product_id and line.product_id.barcode_line_ids:
                                for barcode_line in line.product_id.barcode_line_ids:
